...
from utils.data_loader import inventory
...
def layout():
    ...
    export_section = add_export_section({
        "Inventory_Complete": inv,
        "Critical_Items": inv[inv["status"] == "Critical"] if not inv.empty else None,
        "Low_Stock_Items": inv[inv["status"] == "Low"] if not inv.empty else None
    })
    ...
    fig_status = go.Figure(go.Pie(
        ...
        hole=0.62, marker=dict(colors=[GREEN, AMBER, RED]),
        textinfo="label+value", textfont=dict(color="#f0fdf4", size=12),
    ))
    ...
    for _, row in critical_items.iterrows():
        ...
        fig_bars.add_trace(go.Bar(
            name=row["item_name"][:25], x=[pct], y=[row["item_name"][:25]],
            orientation="h", marker=dict(color=RED if pct < 20 else AMBER),
            text=[f"{pct:.0f}% — {row['farm_name'][:15]}"], textfont=dict(color="#f0fdf4", size=10),
        ))
    apply_theme(fig_bars, 300)
    fig_bars.update_layout(title=dict(text="Critical Stock Items — % of Capacity", font=dict(color="#86efac", size=13)), showlegend=False, xaxis=dict(range=[0, 100]))
    ...
    table = html.Table([
        html.Thead(html.Tr([html.Th(h, style={"color": "#4ade80", "fontSize": "0.70rem", "fontWeight": "600", "textTransform": "uppercase", "padding": "8px 6px"}) for h in ["Farm", "Item", "Category", "Qty / Max", "Unit", "Status", "Supplier"]])),
        html.Tbody(rows),
    ...
    return html.Div([
        ...
        card([html.Div("⚠️ Low & Critical Stock Items", style={"color": AMBER, "fontWeight": "600", "marginBottom": "14px", "fontSize": "0.9rem"}), html.Div(table, style={"overflowX": "auto", "maxHeight": "400px", "overflowY": "auto"})]),
    ])
