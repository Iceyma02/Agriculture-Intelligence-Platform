...
from utils import economic, weather
from utils import GREEN, AMBER, RED, LIME, BLUE, PURPLE, apply_theme, page_header, card, kpi
...
def layout():
    ...
    for col, label, color in [("global_maize_usd_ton", "Maize", GREEN), 
                               ("global_wheat_usd_ton", "Wheat", AMBER), 
                               ("global_soya_usd_ton", "Soya", BLUE)]:
    ...
    if "month" in ec.columns and "electricity_availability_pct" in ec.columns:
        monthly_ec = ec.groupby("month").agg(
            ...
            drought=("drought_index", "mean") if "drought_index" in ec.columns else ("electricity_availability_pct", "mean"),
        ).reset_index()
    ...
    if "month" in ec.columns and "drought_index" in ec.columns:
        monthly_ec = ec.groupby("month").agg(drought=("drought_index", "mean")).reset_index()
    ...
    return html.Div([
        ...
        card([
            ...
            html.Div(weather_table, style={"overflowX": "auto"}),
        ]),
    ])
