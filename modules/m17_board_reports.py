...
from utils.data_loader import board, pnl, farms, losses, marketing
from utils.helpers import GREEN, AMBER, RED, LIME, BLUE, PALETTE, fmt_usd, fmt_tons, apply_theme, page_header, card, kpi, add_export_section, create_empty_chart
...
def layout():
    ...
    fig_margin = go.Figure(go.Scatter(
        ...
        mode="lines+markers+text", line=dict(color=LIME, width=3), marker=dict(size=12, color=LIME),
        text=[f"{v:.1f}%" for v in br["avg_profit_margin_pct"]], textposition="top center", textfont=dict(color="#f0fdf4"),
    ))
    ...
    if not p.empty and "season" in p.columns and latest_season in p["season"].values:
        ...
        fig_top5 = go.Figure(go.Bar(
            ...
            marker=dict(color=PALETTE[:5]), text=[fmt_usd(v) for v in top5["gross_profit_usd"]],
        ...
        fig_top5 = create_empty_chart("No profit data available", "Top Farms by Profit")
    ...
    risk_table = html.Table([
        html.Thead(html.Tr([html.Th(h, style={"color": "#4ade80", "fontSize": "0.70rem", "fontWeight": "600", "textTransform": "uppercase", "padding": "8px"}) for h in ["Risk Item", "Level", "Current Impact", "Recommended Action"]])),
        html.Tbody(risk_rows),
    ], style={"width": "100%", "borderCollapse": "collapse"})

    # Executive summary
    exec_summary = html.Div([
        html.Div(f"📋 BOARD EXECUTIVE SUMMARY — {latest_season} SEASON", style={"fontFamily": "'Playfair Display',serif", "fontSize": "1.0rem", "fontWeight": "700", "color": GREEN, "marginBottom": "20px"}),
        html.Div([
            html.Div([
            ...
            html.Div([
                ...
                *[html.P(f"• {rec}", style={"color": "#86efac", "fontSize": "0.82rem", "lineHeight": "1.7", "marginBottom": "6px"}) for rec in latest["recommendations"].split("; ")],
    ...
    return html.Div([
        ...
        card([html.Div("⚠️ Risk Register — Current Season", style={"color": RED, "fontWeight": "600", "marginBottom": "14px", "fontSize": "0.9rem"}), risk_table]),
    ])
