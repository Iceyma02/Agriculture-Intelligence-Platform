...
def layout():
    ...
    fig_rev = go.Figure(go.Bar(
        x=last12["farm_name"], y=last12["revenue"],
        marker=dict(color=PALETTE[:len(last12)]),
        text=[fmt_usd(v) for v in last12["revenue"]],
        textfont=dict(color="#f0fdf4", size=10),
    ))
    apply_theme(fig_rev, 280)
    fig_rev.update_layout(title=dict(text="Revenue by Farm — Last 12 Months", font=dict(color="#86efac", size=13)),
                           xaxis_tickangle=-25)

    fig_yield = go.Figure(go.Bar(
        x=last12["farm_name"], y=last12["yield_tons"],
        marker=dict(color=LIME, opacity=0.85),
        text=[fmt_tons(v) for v in last12["yield_tons"]],
        textfont=dict(color="#f0fdf4", size=10),
    ))
    ...
    for i, fn in enumerate(farms_top4):
        ...
        fig_trend.add_trace(go.Scatter(
            x=d["month_label"], y=d["revenue_usd"],
            mode="lines", name=fn[:22],
    ...
    return html.Div([
        ...
        html.Div([
            card([dcc.Graph(figure=fig_rev, config={"displayModeBar": False})]),
            card([dcc.Graph(figure=fig_yield, config={"displayModeBar": False})]),
