...
from utils.data_loader import losses
from utils.helpers import GREEN, AMBER, RED, LIME, BLUE, fmt_usd, fmt_tons, apply_theme, page_header, card, kpi, add_export_section, create_empty_chart

def layout():
    ls = losses()
    
    export_section = add_export_section({"Post_Harvest_Losses_Complete": ls})
    
    if ls.empty:
        return html.Div([
            export_section,
            page_header("Post-Harvest Loss Tracker", "Spoilage · Theft · Pest damage · Transport loss · Grading downgrades"),
            card([html.Div("⚠️ No loss data available", style={"color": AMBER, "textAlign": "center", "padding": "40px"})])
        ])

    total_loss_val = ls["loss_value_usd"].sum()
    total_loss_tons = ls["loss_tons"].sum()
    avg_loss_pct = ls["loss_pct"].mean()
    recoverable_val = ls[ls["recoverable"] == True]["loss_value_usd"].sum() if "recoverable" in ls.columns else 0
    ...
    fig_treemap = go.Figure(go.Treemap(
        labels=by_cause["loss_cause"], parents=[""] * len(by_cause), values=by_cause["total_value"],
        ...
        marker=dict(colorscale=[[0, "#052e16"], [0.4, "#f59e0b"], [1, "#ef4444"]], colors=by_cause["total_value"], showscale=False),
        texttemplate="<b>%{label}</b><br>$%{value:,.0f}", textfont=dict(color="#f0fdf4", size=12),
    ))
    apply_theme(fig_treemap, 320)
    fig_treemap.update_layout(title=dict(text="Loss Value by Cause — Treemap", font=dict(color="#86efac", size=13)), margin=dict(l=0, r=0, t=40, b=0))
    ...
    fig_crop = go.Figure(go.Bar(
        x=by_crop["loss_value_usd"], y=by_crop["crop"], orientation="h",
        marker=dict(color=by_crop["loss_value_usd"], colorscale=[[0, "#1a2e1a"], [1, "#ef4444"]]),
        text=[fmt_usd(v) for v in by_crop["loss_value_usd"]], textfont=dict(color="#f0fdf4", size=10),
    ))
    ...
    fig_farm = go.Figure(go.Bar(
        ...
        marker=dict(color=RED, opacity=0.8), text=[fmt_usd(v) for v in by_farm["total_loss"]],
    ...
    fig_season = go.Figure(go.Bar(
        ...
        marker=dict(color=[AMBER, RED, AMBER]), text=[fmt_usd(v) for v in by_season["loss_value_usd"]],
    ...
    for _, row in by_farm.head(12).iterrows():
        rows.append(html.Tr([
            ...
            html.Td("Cold storage upgrade / Fumigation" if "Storage" in row["worst_cause"] else
                    ...
                    "Re-grading protocol review", style={"color": "#6b7280", "fontSize": "0.78rem", "padding": "9px 6px"}),
        ], style={"borderBottom": "1px solid rgba(34,197,94,0.07)"}))

    table = html.Table([
        html.Thead(html.Tr([html.Th(h, style={"color": "#4ade80", "fontSize": "0.70rem", "fontWeight": "600", "textTransform": "uppercase", "padding": "8px 6px"}) for h in ["Farm", "Total Loss Value", "Avg Loss %", "Primary Cause", "Recommended Intervention"]])),
        html.Tbody(rows),
    ...
    return html.Div([
        ...
        card([html.Div("💔 Loss Analysis & Intervention Recommendations", style={"color": RED, "fontWeight": "600", "marginBottom": "14px", "fontSize": "0.9rem"}), html.Div(table, style={"overflowX": "auto"})]),
    ])
