...
from utils.data_loader import supply_chain
...
def layout():
    ...
    export_section = add_export_section({"Supply_Chain_Complete": sc})
    ...
    fig_funnel = go.Figure(go.Funnel(
        ...
        textinfo="value+percent initial", marker=dict(color=STAGE_COLORS[:len(by_stage)]),
    ...
    fig_dest = go.Figure(go.Bar(
        ...
        text=[fmt_usd(v) for v in dest["value_usd"]], textfont=dict(color="#f0fdf4", size=10),
    ))
    ...
    for _, row in sc.sort_values("start_date", ascending=False).head(25).iterrows():
        rows.append(html.Tr([
            ...
            html.Td(f"+{row['days_delayed']}d" if row["days_delayed"] > 0 else "On time", style={"color": RED if row["days_delayed"] > 0 else GREEN, "fontSize": "0.78rem", "fontWeight": "600", "padding": "8px 6px"}),
        ], style={"borderBottom": "1px solid rgba(34,197,94,0.07)"}))

    table = html.Table([
        html.Thead(html.Tr([html.Th(h, style={"color": "#4ade80", "fontSize": "0.70rem", "fontWeight": "600", "textTransform": "uppercase", "padding": "8px 6px"}) for h in ["Shipment ID", "Farm", "Crop", "Qty", "Stage", "Destination", "Delay"]])),
        html.Tbody(rows),
    ...
    return html.Div([
        ...
        card([html.Div("🔗 Active Shipments", style={"color": "#86efac", "fontWeight": "600", "marginBottom": "14px", "fontSize": "0.9rem"}), html.Div(table, style={"overflowX": "auto", "maxHeight": "380px", "overflowY": "auto"})]),
    ])
